...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'editBed') {
      if (!['BOSS', 'FINANCE'].includes(session.role)) return NextResponse.json({ error: '只有老板/财务可以修改' }, { status: 403 });
      return await editBed(body);
    }
...
async function createBed(body: any, session: any) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // Process fabric items（只记录理论用量，不扣减库存）
    let fabricCost = 0;
    ...
    if (fabricItems?.length) {
      for (const fi of fabricItems) {
        ...
        const fabric = await tx.fabric.findUnique({ where: { id: fi.fabricId }, include: { colors: true } });
        if (!fabric) throw new Error('布料不存在');

        // 若前端直接给了 totalUsed 用之(直接填总量模式)；否则用 layerLength×layerCount
        let totalUsedM: number;
        if (fi.totalUsed !== undefined && fi.totalUsed !== null && Number(fi.totalUsed) > 0) {
          totalUsedM = Number(fi.totalUsed);
          if (fi.totalUsedUnit === '码') totalUsedM = totalUsedM * 0.9144;
        } else {
          let layerLenM = Number(fi.layerLength);
          if (fi.layerLengthUnit === '码') layerLenM = layerLenM * 0.9144;
          totalUsedM = layerLenM * Number(fi.layerCount);
        }

        // 单价+染费：优先用fabricColorId锁定颜色的价+染费，回退到主布
        let unitPrice = Number(fabric.unitPriceM);
        let dyePerUnit = Number(fabric.dyePerUnitM);
        if (fi.fabricColorId) {
          const fc = fabric.colors.find((c) => c.id === fi.fabricColorId);
          if (fc) {
            if (fc.unitPriceM != null) unitPrice = Number(fc.unitPriceM);
            dyePerUnit = Number(fc.dyePerUnitM);
          }
        }
        const effectivePrice = unitPrice + dyePerUnit;

        const itemCost = totalUsedM * effectivePrice;
        fabricCost += itemCost;
        processedFabrics.push({
          fabricId: fi.fabricId, fabricColorId: fi.fabricColorId || null, role: fi.role || '主布',
          layerLength: Number(fi.layerLength) || 0, layerCount: Number(fi.layerCount) || 1,
          totalUsed: totalUsedM, unitPriceM: effectivePrice, totalCost: itemCost,
        });
      }
    }
...
// 老板修改裁床数据
async function editBed(body: any) {
  const { bedId, sizes, fabricItems, materialItems, note } = body;
  if (!bedId) return NextResponse.json({ error: '缺少裁床ID' }, { status: 400 });

  const result = await prisma.$transaction(async (tx) => {
    const bed = await tx.cuttingBed.findUnique({
      where: { id: bedId },
      include: { style: true },
    });
    if (!bed) throw new Error('裁床不存在');

    // 更新尺码数量
    if (sizes?.length) {
      await tx.bedSizeItem.deleteMany({ where: { bedId } });
      await tx.bedSizeItem.createMany({
        data: sizes.map((s: any) => ({ bedId, size: s.size, quantity: Number(s.quantity) || 0 })),
      });
    }

    const totalPieces = sizes ? sizes.reduce((sum: number, s: any) => sum + (Number(s.quantity) || 0), 0) : bed.totalPieces;

    // 更新布料用量（理论用量，不扣库存）
    if (fabricItems) {
      await tx.bedFabricItem.deleteMany({ where: { bedId } });
      let fabricCost = 0;
      for (const fi of fabricItems) {
        if (!fi.fabricId) continue;
        const fabric = await tx.fabric.findUnique({ where: { id: fi.fabricId }, include: { colors: true } });
        ...
        const itemCost = totalUsedM * effectivePrice;
        fabricCost += itemCost;
        await tx.bedFabricItem.create({
          data: {
            bedId, fabricId: fi.fabricId, fabricColorId: fi.fabricColorId || null, role: fi.role || '主布',
            layerLength: Number(fi.layerLength) || 0, layerCount: Number(fi.layerCount) || 1,
            totalUsed: totalUsedM, unitPriceM: effectivePrice, totalCost: itemCost,
          },
        });
      }
      await tx.cuttingBed.update({ where: { id: bedId }, data: { fabricCost } });
    }

    // 更新辅料需求
    if (materialItems) {
      await tx.materialDemand.deleteMany({ where: { bedId } });
      let materialCost = 0;
      for (const mi of materialItems) {
        if (!mi.materialId) continue;
        const mat = await tx.material.findUnique({ where: { id: mi.materialId } });
        if (!mat) continue;
        const needed = Number(mi.qtyPerPiece) * totalPieces;
        materialCost += needed * Number(mat.unitPrice);
        await tx.materialDemand.create({
          data: {
            bedId, materialId: mi.materialId, styleId: bed.styleId,
            bedNo: bed.bedNo, qtyNeeded: needed,
            unit: mi.unit || mat.unit, role: mi.role || null,
          },
        });
      }
      await tx.cuttingBed.update({ where: { id: bedId }, data: { materialCost } });
    }

    // 重算成本
    const updatedBed = await tx.cuttingBed.findUnique({ where: { id: bedId } });
    if (updatedBed) {
      const fc = Number(updatedBed.fabricCost);
      const mc = Number(updatedBed.materialCost);
      const lc = Number(bed.style.laborCost) * totalPieces;
      const pc = Number(bed.style.packCost) * totalPieces;
      const sc = Number(bed.style.shipCost) * totalPieces;
      const dwc = Number(updatedBed.defectWasteCost);
      const total = fc + mc + lc + pc + sc + dwc;
      const unit = totalPieces > 0 ? total / totalPieces : 0;

      await tx.cuttingBed.update({
        where: { id: bedId },
        data: { totalPieces, laborCost: lc, packCost: pc, shipCost: sc, totalCost: total, unitCost: unit, note: note ?? bed.note },
      });
    }
  ...
  return NextResponse.json({ success: true, message: `裁床已更新（${result.totalPieces}件）` });
}
